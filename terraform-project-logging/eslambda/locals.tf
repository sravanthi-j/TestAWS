locals {
  ...
  service = var.service != null ? var.service : var.lambda_name

  # Shared name of this module. Should be used as a prefix for submodules
  name = var.service != null ? "${var.service}-${var.lambda_name}" : var.lambda_name

  # Full module name with all prefixes (to be used as resource names)
  name_full = "${local.module_prefix}${var.project_name != null ? "${var.project}-" : ""}${local.name}"
 ...
 # Map of tags that should be used by submodules
  ...
  tags = merge(local.tags_common, {
    ...
    format(local.shared_tag, "role")             = "Lambda"
  })
